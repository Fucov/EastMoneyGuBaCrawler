...
class guba_comments:
    ...
    def get_data(self, page, content_type="news"):
                ...
                if data_json:
                    try:
                        ...
                        try:
                            ...
                            if (
                                self.secCode not in data_json["url"]
                                and "zssh000001" in data_json["url"]
                            ):
                            ...
                            if (
                                ...
                                is_retry = True
                                raise ContentChangedException("内容重复")
                            if e.details.get("writeErrors") and any(
                                err.get("code") == 11000
                        ...
                        self.logger.error(
                            f"插入数据失败: {e}, url: {data_json.get('url', '无url')}"
                        )
        ...
        self.logger.info(
            f"页面 {page} 处理完成，成功插入 {success_count}/{len(data_list)} 条数据"
        )
        return exist_count
    ...
    def main(self):
        ...
        for content_type in content_types:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                # 使用tqdm显示进度
                with tqdm(
                    total=total_pages, desc=f"{self.secCode} {type_names[content_type]}"
                ) as pbar:
