...
class MongoDBClient:
    ...
    def __init__(self, config: configparser.ConfigParser, db_name: str, collection_name: str):
        ...
        if self.username and self.password:
            ...
            # 构建URI连接字符串
            uri = f"mongodb://{encoded_username}:{encoded_password}@{self.host}:{self.port}/{self.db_name}"
...
class RedisClient:
    ...
    def __init__(self, config: configparser.ConfigParser):
        # 兼容新旧两种配置格式
        self.host = config.get('Redis', 'host', fallback=None) or config.get('Redis', 'redis_host', fallback='localhost')
        self.port = config.getint('Redis', 'port', fallback=None) or config.getint('Redis', 'redis_port', fallback=6379)
        self.password = config.get('Redis', 'password', fallback=None) or config.get('Redis', 'redis_password', fallback=None)
        self.db = config.getint('Redis', 'db', fallback=None) or config.getint('Redis', 'redis_db', fallback=0)
        self.redis_key = config.get('Redis', 'url_queue_key', fallback=None) or config.get('Redis', 'redis_key', fallback='urls')
